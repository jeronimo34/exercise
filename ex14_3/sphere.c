...
CreateSphereData(GLfloat center[], GLfloat radius, GLint number_subdivisions) {
  ...
  for (i = 0; i < number_subdivisions; i++) {
    GLfloat u = M_PI * (GLfloat)i/(number_subdivisions-1) - 0.5 * M_PI;
    for (j = 0; j < number_subdivisions; j++) {
      GLfloat v = 2.0 * M_PI * (GLfloat)j/(number_subdivisions-1);

      normal[i][j][0] = cos(v)*cos(u);
      normal[i][j][1] = sin(v)*cos(u);
      normal[i][j][2] = sin(u);
...
static void LoadShaders(void) {
  g_program = BuildProgram("light.vs.glsl", "light.fs.glsl");
  ...
  g_uniforms.light_position = glGetUniformLocation(g_program, "lightPosition");
  g_uniforms.material_color = glGetUniformLocation(g_program, "materialColor");
  g_uniforms.light_color = glGetUniformLocation(g_program, "lightColor");
...
static void InitLights() {
  // Position of light 0
  // {0.0, 2.5, 10.0, 1.0}
  g_scene_data.light_position[0] = 0.0;
  g_scene_data.light_position[1] = 0.0;
  g_scene_data.light_position[2] = 10.0;
  g_scene_data.light_position[3] = 1.0;
}


static void InitMaterial() {
  g_scene_data.material_color[0] = 1.0;
  g_scene_data.material_color[1] = 0.0;
  g_scene_data.material_color[2] = 0.0;
  g_scene_data.material_color[3] = 1.0;
}

static void InitLightColor()
{
	g_scene_data.light_color[0] = 1;
	g_scene_data.light_color[1] = 1;
	g_scene_data.light_color[2] = 1;
	g_scene_data.light_color[3] = 1;
}
...
void HandleDisplay(void) {
  ...
  glUniform4fv(g_uniforms.light_position, 1, &g_scene_data.light_position[0]);
  glUniform4fv(g_uniforms.material_color, 1, &g_scene_data.material_color[0]);  
  glUniform4fv(g_uniforms.light_color, 1, &g_scene_data.light_color[0]);
...
static void ComputeRotationZMatrix(GLfloat a[4][4], GLfloat theta) {
  int i, j;
  GLfloat t = theta * M_PI / 180.0f; // t is in radian
