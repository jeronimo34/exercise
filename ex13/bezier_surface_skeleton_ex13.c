...
/* Globals */
int show_cont_points = 0;
int show_cont_polygon = 0;
...
/* Complete */
	...
	for (int i = 0; i < number_control_points_u; ++i)
		...
		{
			glVertex3dv(control_points[j][i]);
		}
	glEnd();
	glEnable(GL_LIGHTING);
}
...
/* Complete */
	...
	for (int i = 0; i < number_control_points_u; ++i) {
		...
		for (int j = 0; j < number_control_points_v; ++j)
		{
			glVertex3dv(control_points[i][j]);
		}
		glEnd();
	}
	for (int i = 0; i < number_control_points_u; ++i) {
...
static void PlotBezierSurface(void) {
  ...
  glMap2d(GL_MAP2_VERTEX_3, 0, 1, 3, number_control_points_u,
...
static void HandleKey(unsigned char key, int x, int y) {
  switch(key) {
    case 27:
      ...
      break;
	case 'p':
		show_cont_points = !show_cont_points;
		break;
	case 'm':
		show_cont_polygon = !show_cont_polygon;
		break;
	case 'u':
		number_samples_u++;
		break;
