...
enum {POSITION_LOCATION, COLOR_LOCATION};
...
static void BuildGeometry() {
  GLfloat verts[] = {
    // Vertex 1 position
    -0.5f, -0.5f, 
    // Vertex 1 color (yellow)
    1.0f, 1.0f, 0.0f,
    // Vertex 2 position
    +0.0f, +0.5f, 
    // Vertex 2 color (magenta)
    1.0f, 0.0f, 1.0f, 
    // Vertex 3 position
    +0.5f, -0.5f, 
    // Vertex 3 color (cyan)
  ...
  g_stride = 5 * sizeof(GLfloat);
  GLenum usage = GL_STATIC_DRAW;
  g_color_offset = (GLvoid*)(sizeof(GLfloat) * 2);
...
static void LoadShaders() {
  ...
  vsSource = GetShaderSource("simple2.vs.glsl");
  ...
  fsSource = GetShaderSource("simple2.fs.glsl");
  ...
  free(fsSource);
  if (success == GL_FALSE) {
    fprintf(stderr, "Error in compiling: simple2.fs.glsl\n");
    fprintf(stderr, "Error message: %s\n", message);
    exit(1);
  }
  ...
  glBindAttribLocation(g_programHandle, COLOR_LOCATION, "vColor");
...
static void HandleDisplay(void) {
  ...
  glVertexAttribPointer(COLOR_LOCATION, 3, GL_FLOAT, GL_FALSE, g_stride, g_color_offset);
